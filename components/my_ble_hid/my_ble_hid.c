...
#define MY_BLE_HID_DEV_NAME "ESP32S3BLEKB"
static char my_ble_kb_name[sizeof(MY_BLE_HID_DEV_NAME) + sizeof(my_chip_macstr_short) + 1] = MY_BLE_HID_DEV_NAME;
typedef struct
{
...
esp_err_t my_ble_hid_start(void)
{
    ...
    if (!my_ble_inited) {
        ...
        snprintf(my_ble_kb_name, sizeof(my_ble_kb_name), "%s_%s", MY_BLE_HID_DEV_NAME, my_chip_macstr_short);
        ...
        ble_svc_gap_device_name_set(MY_BLE_HID_DEV_NAME);
        ble_svc_gap_device_appearance_set(ESP_HID_APPEARANCE_GENERIC);
