...
static const char *TAG = "my esp hid gap";

#pragma region bluedroid ble
 ...
 * BLE GAP
 * */
extern void my_ble_hidd_usable_event_cb(void);
...
#if !CONFIG_BT_NIMBLE_ENABLED
esp_err_t my_esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
...
esp_err_t my_esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    // ble_hs_cfg.sm_io_cap = BLE_SM_IO_CAP_DISP_ONLY; //显示配对码，要在主机输入才能连接
    ble_hs_cfg.sm_io_cap = BLE_SM_IO_CAP_NO_IO; // 不需要任何验证行为
    // ble_hs_cfg.sm_io_cap = BLE_SM_IO_CAP_DISP_YES_NO; // 连接似乎不稳定，主机端会弹窗提示是否接受配对码，我们假设设备端没有输入输出能力，所以设备端始终接受，但会在设备端打印配对码，用户可以进行比对
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0)
        {
            ...
            ESP_LOGD(TAG, "BLE_GAP_EVENT_CONNECT conn_itvl: %d, conn_latency: %d ,supervision_timeout: %d", desc.conn_itvl, desc.conn_latency, desc.supervision_timeout);
            if (desc.conn_itvl > 6 || desc.conn_latency > 0)
            {
                struct ble_gap_upd_params upd_params = {
                    ...
                    .min_ce_len = 1,
                    .max_ce_len = 0x01FF};
                rc = ble_gap_update_params(event->connect.conn_handle, &upd_params);
                if (rc != 0)
                {
                    ESP_LOGW(TAG, "ble_gap_update_params error rc: %d", rc);
                    return rc;
                }
            }
        }
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ESP_LOGD(TAG, "BLE_GAP_EVENT_DISCONNECT; reason=%d", event->disconnect.reason);
        return 0;
    case BLE_GAP_EVENT_CONN_UPDATE:
        /* The central has updated the connection parameters. */
        ESP_LOGD(TAG, "BLE_GAP_EVENT_CONN_UPDATE; status=%d", event->conn_update.status);
        return 0;
    case BLE_GAP_EVENT_CONN_UPDATE_REQ:
        ESP_LOGD(TAG, "BLE_GAP_EVENT_CONN_UPDATE_REQ");
        // if (event->conn_update_req.peer_params)
        // {
        //     const struct ble_gap_upd_params *p = event->conn_update_req.peer_params;
        //     ESP_LOGW(TAG, "peer params: min_int=%d, max_int=%d, latency=%d, timeout=%d,mincelen:%d, max:%d", p->itvl_min, p->itvl_max, p->latency, p->supervision_timeout, p->min_ce_len, p->max_ce_len);
        // }
        // else
        // {
        //     ESP_LOGE(TAG, "peer params is null");
        // }
        // if (event->conn_update_req.self_params)
        // {
        //     struct ble_gap_upd_params *p = event->conn_update_req.self_params;
        //     ESP_LOGW(TAG, "self params: min_int=%d, max_int=%d, latency=%d, timeout=%d,mincelen:%d, max:%d", p->itvl_min, p->itvl_max, p->latency, p->supervision_timeout, p->min_ce_len, p->max_ce_len);
        //     // rc = ble_gap_update_params(event->conn_update_req.conn_handle, p);
        //     // ESP_LOGI(TAG, "ble_gap_update_params rc: %d", rc);
        // }
        // else
        // {
        //     ESP_LOGW(TAG, "self_params: null");
        // }
        return 0;
    ...
    case BLE_GAP_EVENT_MTU:
        ESP_LOGD(TAG, "mtu update event; conn_handle=%d cid=%d mtu=%d", event->mtu.conn_handle, event->mtu.channel_id, event->mtu.value);
        return 0;
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP)
        {
            ...
            ESP_LOGD(TAG, "ble_sm_inject_io result: %d", rc);
        }
        else if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) // 数字比较，对应BLE_SM_IO_CAP_DISP_YES_NO
        {
            // ESP_LOGI(TAG, "Accepting passkey..");
            pkey.action = event->passkey.params.action;
            pkey.numcmp_accept = 1;
            rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
            ESP_LOGI(TAG, "please compare the passkey: %ld", event->passkey.params.numcmp);
...
static uint8_t my_ble_gap_inited = 0;
esp_err_t my_esp_hid_gap_init(uint8_t mode)
{
    esp_err_t ret;
    if (!mode || mode > ESP_BT_MODE_BTDM || my_ble_gap_inited)
    {
        ESP_LOGE(TAG, "Invalid mode given! or inited");
        return ESP_FAIL;
    }
#if !CONFIG_BT_NIMBLE_ENABLED
...
esp_err_t my_esp_hid_gap_deinit()
{
    if (my_ble_gap_inited)
    {
        esp_err_t ret = esp_nimble_deinit();
        if (ret)
        {
            ESP_LOGE(TAG, "esp_nimble_deinit() failed");
        }

        ret = esp_bt_controller_disable();
        if (ret)
        {
            ESP_LOGE(TAG, "esp_bt_controller_disable() failed");
        }
