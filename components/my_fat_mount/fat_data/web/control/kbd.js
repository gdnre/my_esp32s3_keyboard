...
function renderMatrixGrid(layerConfigs) {
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            ...
            const x = (startPos.x + col * spacing) * PHYSICAL_LAYOUT.width;
            const y = (startPos.y + row * spacing) * PHYSICAL_LAYOUT.height;
            const keyConfig = findKeyConfig(layerConfigs, id);
            if (!keyConfig) continue;
                    ...
                    <div class="value-selector">
                        <input type="text" class="value-search" 
                            id="ivalue_${ioType}_${layer}_${keyId}" name="ivalue_${ioType}_${layer}_${keyId}"
                               ...
                               value="${getValueDisplay(keyConfig)}">
                        <select class="key-value-select" data-key-id="${keyId}"
                        id="svalue_${ioType}_${layer}_${keyId}" svalue="type_${ioType}_${layer}_${keyId}" >
                            ${getValueOptions(keyConfig.type, keyConfig.value)}
                        </select>
                    </div>
                </div>
            ` : ''}
...
function bindPanelEvents(panel, originalConfig) {
    ...
    const valueSelect = panel.querySelector('.key-value-select');
    ...
    if (valueSearch) {
        valueSearch.addEventListener('input', (e) => {
            filterValueOptions(e.target.value, valueSelect);
        });
    ...
    if (valueSelect) {
        ...
        valueSelect.addEventListener('click', (e) => {
            e.stopPropagation();
        });
    }
...
function updateKeyType(newType) {
    const keyId = parseInt(activeKeyElement.dataset.id);
    const layer = parseInt(activeKeyElement.dataset.layer);

    let keyConfig = findKeyConfig(currentKbdConfig.keyConfigsArr, keyId, layer);
    if (!keyConfig) {
        // 创建新配置
        const ioType = findIoType(keyId, layer);
        keyConfig = { id: keyId, type: newType };
        const layerConfig = currentKbdConfig.keyConfigsArr.find(
...
function updateKeyValue(newValue) {
    const keyId = parseInt(activeKeyElement.dataset.id);
    const layer = parseInt(activeKeyElement.dataset.layer);
    const keyConfig = findKeyConfig(currentKbdConfig.keyConfigsArr, keyId, layer);
...
function getValueDisplay(keyConfig) {
    if (!keyConfig || keyConfig.value === undefined) return '';

    const typeInfo = KEY_CONFIG_SCHEMA.keyTypes[keyConfig.type];
    const valueLabel = KEY_CONFIG_SCHEMA.valueLabels[typeInfo.valueType]?.[keyConfig.value];
    return valueLabel || keyConfig.value.toString();
}

function getValueOptions(type, currentValue) {
    const typeInfo = KEY_CONFIG_SCHEMA.keyTypes[type];
    if (!typeInfo?.hasValue) return '';

    const valueType = typeInfo.valueType;
    const labels = KEY_CONFIG_SCHEMA.valueLabels[valueType] || {};
...
function filterValueOptions(searchTerm, selectEl) {
...
function findIoType(keyId, layer) {
    for (const config of currentKbdConfig.keyConfigsArr) {
        if (config.layer === layer && config.keysArr.some(k => k.id === keyId)) {
            return config.ioType;
        }
    }
    // 默认返回第一个IO类型
    return currentKbdConfig.keyConfigsArr[0]?.ioType || 0;
}
