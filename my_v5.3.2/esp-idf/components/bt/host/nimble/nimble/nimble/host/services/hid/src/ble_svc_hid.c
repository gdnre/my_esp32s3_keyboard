...
void fill_boot_kbd_inp(uint8_t instance)
{
    struct ble_gatt_chr_def *chr, demo_chr;
...
void fill_boot_kbd_out(uint8_t instance)
{
    struct ble_gatt_chr_def *chr, demo_chr;
...
void fill_boot_mouse_inp(uint8_t instance)
{
    struct ble_gatt_chr_def *chr, demo_chr;
...
static my_nimble_report_cb my_output_report_cb = NULL;
int8_t my_nimble_register_report_cb(uint8_t report_type, my_nimble_report_cb cb)
{
    my_output_report_cb = cb;
    return 1;
}
/**
 * HID access function
    ...
    for (int instance = 0; instance < instances; instance++)
    {
        ...
        case BLE_SVC_HID_CHR_UUID16_RPT:
            ...
            else if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR)
            {
                ...
                rpt->len = out_rpt_len;
                if (rpt->len && my_output_report_cb)
                {
                    my_output_report_cb(rpt->type, rpt->id, rpt->data, rpt->len);
                }
                if (ctxt->chr->flags & BLE_GATT_CHR_F_NOTIFY)
                {
                    // notify类型才会执行到这里，output report不会执行到这
                    ble_gatts_chr_updated(*(ctxt->chr->val_handle));
                }
                return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
            }
